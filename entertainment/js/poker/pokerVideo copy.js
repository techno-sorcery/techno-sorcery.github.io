...
function betDraw(){
	if(!running){
		...
		for(let i=0;i<5;i++){
			...
			if(!held[i]) document.getElementById('card'.concat(i+1)).src = "js/cards/BLUE_BACK.svg";
		}
		setTimeout(function() {
...
function reveal(){
	let newCards = [];
	for(let i=0;i<5;i++){
				document.getElementById('card'.concat(i+1)).src = "js/cards/".concat(cardParse(cards[i]),'.svg');
	}
	pingAudio.play();
